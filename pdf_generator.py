...
def safe(val, fallback='—'):
    if val is None or str(val).strip().lower() in ('','null','none'): return fallback
...
class DestinationBar(Flowable):
    def __init__(self, destination):
        super().__init__(); self.width=CW; self.height=14*mm; self.destination=destination
    def draw(self):
        c=self.canv; c.setFillColor(SAND); c.roundRect(0,0,self.width,self.height,3,fill=1,stroke=0)
...
class HeaderBand(Flowable):
    def __init__(self,agency,tagline='TRAVEL & SAFARI SPECIALISTS'):
        super().__init__(); self.width=CW; self.height=20*mm
...
class InternalHeaderBand(Flowable):
    def __init__(self,agency):
        super().__init__(); self.width=CW; self.height=20*mm; self.agency=agency.upper()
    def draw(self):
        c=self.canv; w,h=self.width,self.height; pad=5*mm
...
def build_page1(data,S,story):
    agent=data.get('agent',{}); client=data.get('client',{}); trip=data.get('trip',{}); profile=data.get('agent_profile',{})
    agency=safe(agent.get('agency'),'Safari Agency'); agent_name=safe(agent.get('name')); agent_email=safe(agent.get('email'))
    tagline=safe(profile.get('tagline'),'Travel & Safari Specialists'); quote_id=safe(data.get('quote_id'),'SF-0001'); gen_date=safe(data.get('generated_at'))
    ...
    start=safe(trip.get('start_date')); end=safe(trip.get('end_date')); nights=safe(trip.get('duration_nights')); dests=safe(trip.get('destinations'))
...
def build_itinerary(data,S,story):
    ...
    for day in itinerary:
        ...
        narr=narr_idx.get(num,{}); dest=safe(day.get('destination')); title=safe(day.get('title'),dest)
        img_query=safe(day.get('image_search_query'),f"{dest} safari"); narr_text=safe(narr.get('narrative'),'')
        highlight=safe(narr.get('highlight'),''); accom=safe(day.get('accommodation_name'))
        accom_desc=safe(narr.get('accommodation_description'),''); room=safe(day.get('room_type'))
        meals=safe(day.get('meal_plan')); transport=safe(day.get('transport_description'),'')
        elems=[]
...
def build_inclusions_terms(data,S,story):
    ...
    if inclusions and inclusions!='—':
        le+=[Spacer(1,2*mm),Paragraph('INCLUDED IN THIS PACKAGE',S['section']),hr(width=col_w-8*mm,color=HexColor('#4A9E4A'))]
        for line in inclusions.split('\n'):
            line=line.strip().lstrip('-•✓').strip()
            if line: le.append(Paragraph(f'<font color="#2D6B2D">✓</font>  {line}',S['body_sm']))
        le.append(Spacer(1,2*mm))
    if exclusions and exclusions!='—':
        re+=[Spacer(1,2*mm),Paragraph('NOT INCLUDED',S['section']),hr(width=col_w-8*mm,color=HexColor('#C94040'))]
        for line in exclusions.split('\n'):
            line=line.strip().lstrip('-•✕✗').strip()
            if line: re.append(Paragraph(f'<font color="#B03030">✕</font>  {line}',S['body_sm']))
        re.append(Spacer(1,2*mm))
    if le or re:
        inc_t=Table([[le,re]],colWidths=[col_w,col_w])
...
def build_trust_page(data,S,story):
    ...
    cw2=(CW-6*mm)/2; ale=[]; are=[]
    if awards: ale+=[Paragraph('AWARDS & RECOGNITION',S['section']),hr(width=cw2)]+[Paragraph(f"■  {safe(a)}",S['award']) for a in awards]+[Spacer(1,3*mm)]
    if memberships: are+=[Paragraph('MEMBERSHIPS & ACCREDITATIONS',S['section']),hr(width=cw2)]+[Paragraph(f"✓  {safe(m)}",S['award']) for m in memberships]+[Spacer(1,3*mm)]
    if ale or are:
        am=Table([[ale,are]],colWidths=[cw2,cw2])
...
def generate_quote_pdf(data:dict, output_path:str):
    """Clean client-facing quote PDF. No cost data."""
    S=make_styles(); agency=safe(data.get('agent',{}).get('agency'),'SafariFlow')
    email=safe(data.get('agent',{}).get('email'),''); date=safe(data.get('generated_at'),'')
...
def generate_cost_breakdown_pdf(data:dict, output_path:str):
    """
    Internal agent document.
    Format: dark header + meta strip + investment breakdown + cost breakdown table + profit summary.
    No itinerary, no overview, no trust page. Never sent to clients.
    """
    S=make_styles(); agent=data.get('agent',{}); client=data.get('client',{}); trip=data.get('trip',{})
    line_items=data.get('line_items',[]); pricing=data.get('pricing',{})
    agency=safe(agent.get('agency'),'SafariFlow'); agent_name=safe(agent.get('name')); agent_email=safe(agent.get('email'))
    quote_id=safe(data.get('quote_id'),'SF-0001'); gen_date=safe(data.get('generated_at'))
    client_name=safe(client.get('name')); pax_adults=safe(client.get('pax_adults'),'2'); pax_kids=safe(client.get('pax_children'),'0')
    pax_str=f"{pax_adults} Adults"+(f", {pax_kids} Children" if pax_kids not in ('0','—') else '')
    ...
    story.append(Paragraph(f"{agency}  ·  {agent_name}  ·  {agent_email}  ·  {gen_date}",S['muted'])); story.append(Spacer(1,5*mm))
    col=CW/4
    ...
    tc=0.0; ts=0.0; tp=0.0
    for item in line_items:
        cost=float(item.get('cost_total_price',item.get('total_price',0)) or 0)
        sell=float(item.get('total_price',0) or 0); markup=float(item.get('markup_pct',0) or 0)
        profit=float(item.get('profit',sell-cost) or 0)
        tc+=cost; ts+=sell; tp+=profit
    ...
    margin=(tp/ts*100) if ts>0 else 0
    cost_rows.append([Paragraph('TOTALS',ParagraphStyle('ctl',fontName='Helvetica-Bold',fontSize=8.5,textColor=white,leading=11)),Paragraph(usd(tc),ParagraphStyle('ctv',fontName='Helvetica-Bold',fontSize=8.5,textColor=white,leading=11,alignment=TA_RIGHT)),Paragraph(usd(ts),ParagraphStyle('ctv2',fontName='Helvetica-Bold',fontSize=8.5,textColor=white,leading=11,alignment=TA_RIGHT)),Paragraph(f"{margin:.1f}%",ParagraphStyle('ctm',fontName='Helvetica-Bold',fontSize=8.5,textColor=GOLD,leading=11,alignment=TA_RIGHT)),Paragraph(usd(tp),ParagraphStyle('ctp',fontName='Helvetica-Bold',fontSize=8.5,textColor=PROFIT_LGT,leading=11,alignment=TA_RIGHT))])
    ...
    summary=Table([[Paragraph('TOTAL COST',S['total_lbl']),Paragraph(usd(tc),S['td_r']),Spacer(6*mm,1),Paragraph('TOTAL REVENUE',S['total_lbl']),Paragraph(usd(ts),S['td_r']),Spacer(6*mm,1),Paragraph('TOTAL PROFIT',ParagraphStyle('pl',fontName='Helvetica-Bold',fontSize=9,textColor=PROFIT_GRN,leading=12,alignment=TA_RIGHT)),Paragraph(usd(tp),ParagraphStyle('pv',fontName='Helvetica-Bold',fontSize=16,textColor=PROFIT_GRN,leading=20,alignment=TA_RIGHT)),Spacer(6*mm,1),Paragraph('MARGIN',ParagraphStyle('ml',fontName='Helvetica-Bold',fontSize=9,textColor=GOLD,leading=12,alignment=TA_RIGHT)),Paragraph(f"{margin:.1f}%",ParagraphStyle('mv',fontName='Helvetica-Bold',fontSize=16,textColor=GOLD,leading=20,alignment=TA_RIGHT))]],colWidths=[28*mm,20*mm,6*mm,28*mm,20*mm,6*mm,28*mm,22*mm,6*mm,18*mm,18*mm])
