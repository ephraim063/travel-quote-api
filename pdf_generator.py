...
def generate_quote_pdf(data: dict, output_path: str):
    ...
    # ── Map Make.com payload fields to internal structure ─────────────────────
    import json
    ...
    # Remap flat Make.com payload into structured sections pdf builder expects
    mapped = {
        'quote_id':       safe(data.get('quote_number'), 'SF-0001'),
        'generated_at':   safe(data.get('quote_date'), ''),
        'accept_url':     safe(data.get('accept_url'), '#'),
        'changes_url':    safe(data.get('changes_url'), '#'),
        'fetch_photos':   False,
        'inclusions':     safe(data.get('inclusions'), ''),
        'exclusions':     safe(data.get('exclusions'), ''),
        'terms':          safe(data.get('notes'), ''),

        'agent': {
            'name':    safe(data.get('agent_name')),
            'email':   safe(data.get('agent_email')),
            'phone':   safe(data.get('agent_phone')),
            'agency':  safe(data.get('agency_name')),
            'logo_url':safe(data.get('agent_logo_url'), ''),
            'website': safe(data.get('agent_website'), ''),
        },

        'client': {
            'name':        safe(data.get('client_name')),
            'email':       safe(data.get('client_email')),
            'pax_adults':  safe(data.get('num_travelers', '2 Adults').split()[0], '2'),
            'pax_children':safe(data.get('pax_children'), '0'),
            'nationality': safe(data.get('client_nationality'), ''),
        },

        'trip': {
            'title':           safe(data.get('destination'), 'Safari'),
            'start_date':      safe(data.get('travel_dates', ' - ').split(' - ')[0]),
            'end_date':        safe(data.get('travel_dates', ' - ').split(' - ')[-1]),
            'duration_nights': safe(data.get('duration_days'), '7'),
            'destinations':    safe(data.get('destination'), ''),
            'travel_style':    safe(data.get('accommodation_tier'), 'Luxury'),
        },

        'itinerary':  parse_if_string(data.get('days', [])),
        'line_items': parse_if_string(data.get('items', [])),

        'pricing': {
            'total_price_usd':    data.get('total_price_usd', 0),
            'deposit_amount_usd': data.get('deposit_amount_usd', 0),
            'balance_amount_usd': data.get('balance_amount_usd', 0),
            'within_budget':      data.get('within_budget', True),
            'budget_notes':       data.get('budget_notes', ''),
        },

        'narrative': {
            'intro': safe(data.get('intro_narrative'), ''),
            'days':  parse_if_string(data.get('narrative_days', [])),
        },
    ...
    agency = safe(mapped['agent'].get('agency'), 'SafariFlow')
    email  = safe(mapped['agent'].get('email'), '')
    date   = safe(mapped.get('generated_at'), '')
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Safari Quote — {safe(mapped['client'].get('name', ''))}",
        author=agency,
    )

    story = []
    build_page1(mapped, S, story)
    build_overview(mapped, S, story)
    build_itinerary(mapped, S, story)
    build_pricing(mapped, S, story)
    build_inclusions_terms(mapped, S, story)
    build_trust_page(mapped, S, story)
