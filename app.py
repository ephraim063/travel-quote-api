# ─────────────────────────────────────────────────────────────────
# SafariDesk — Flask API v3.1
# PDF Generation + Agent Approval + Client Response Flow
# ─────────────────────────────────────────────────────────────────
...
def pdf_to_base64(buffer):
    """Convert PDF BytesIO buffer to base64 string for Make.com"""
...
# Called by Make.com — returns base64 PDF + filename
# ══════════════════════════════════════════════════════════════════
...
def generate_quote():
    """
    Make.com Scenario 1, Module 9
    Generates branded quote PDF with itinerary, pricing, action buttons,
    and agency trust page.
    Returns base64-encoded PDF for Make.com to upload to Supabase Storage.
    """
    try:
        ...
        filename   = f"quote-{data.get('quote_number', 'draft')}.pdf"
        ...
        print(f"[ERROR] generate_quote: {e}")
...
def generate_receipt():
    """
    Make.com Scenario 3, Module 4
    Generates payment receipt PDF after deposit is confirmed.
    """
    try:
        data = request.get_json()
        if not data:
        ...
        pdf_buffer = generate_receipt_pdf(data)
        filename   = f"receipt-{data.get('receipt_number', 'draft')}.pdf"
        ...
        print(f"[ERROR] generate_receipt: {e}")
        return jsonify({"error": str(e)}), 500


@app.route("/generate-voucher/accommodation", methods=["POST"])
@require_api_key
def generate_accom_voucher():
    """
    Make.com Scenario 3, Module 5
    Generates accommodation voucher PDF per property per booking.
    Can be called multiple times (once per property in the itinerary).
    """
    try:
        data = request.get_json()
        if not data:
        ...
        pdf_buffer = generate_accommodation_voucher(data)
        filename   = f"voucher-accom-{data.get('voucher_number', 'draft')}.pdf"
        ...
        print(f"[ERROR] generate_accom_voucher: {e}")
        return jsonify({"error": str(e)}), 500


@app.route("/generate-voucher/transport", methods=["POST"])
@require_api_key
def generate_transport_voucher_route():
    """
    Make.com Scenario 3, Module 6
    Generates transport voucher PDF per transfer/flight per booking.
    Can be called multiple times (once per transport leg in the itinerary).
    """
    try:
        data = request.get_json()
        if not data:
        ...
        pdf_buffer = generate_transport_voucher(data)
        filename   = f"voucher-transport-{data.get('voucher_number', 'draft')}.pdf"
        ...
        print(f"[ERROR] generate_transport_voucher: {e}")
        return jsonify({"error": str(e)}), 500
...
def generate_itinerary():
    """
    Standalone itinerary PDF (backwards compatibility).
    """
    try:
        data = request.get_json()
        if not data:
        ...
        filename   = f"itinerary-{data.get('itinerary_number', 'draft')}.pdf"
        ...
        print(f"[ERROR] generate_itinerary: {e}")
        return jsonify({"error": str(e)}), 500


# ══════════════════════════════════════════════════════════════════
# AGENT APPROVAL ROUTES
# Agent reviews AI-generated quote before it goes to client
# ══════════════════════════════════════════════════════════════════
...
def generate_approval_links():
    """
    Make.com Scenario 1, Module 12 (agent notification email)
    ...
    return jsonify({
        ...
        "edit_url":     f"{PORTAL_URL}/quotes/{quote_number}/edit",
...
def approve_quote():
    """
    Agent clicks APPROVE in their email.
    Fires Make.com Scenario 2 webhook → sends quote to client.
    """
    ...
    return page_html(
        "approved",
        "Quote Approved! ✅",
        f"Quote <b>{quote_number}</b> has been approved and sent to the client.",
        "The client will receive their personalised quote within minutes.",
        show_portal=True,
    )
...
def reject_quote():
    """
    Agent clicks REJECT in their email.
    Step 1: Shows reason form.
    Step 2: On form submit, fires Make.com Scenario 2 with rejected status.
    """
    token  = request.args.get("token")
    ...
    if reason:
        ...
        return page_html(
            "rejected",
            "Quote Rejected",
            f"Quote <b>{quote_number}</b> has been marked as rejected.",
...
# Client receives quote email and clicks Accept / Request Changes
# ══════════════════════════════════════════════════════════════════
...
def generate_client_links():
    """
    Make.com Scenario 2 — after agent approves quote.
    Returns accept/changes URLs to embed in client email.
    Tokens expire in 14 days (matching quote validity).
    """
    data         = request.get_json()
    quote_number = data.get("quote_number")
...
def client_accept():
    """
    Client clicks ACCEPT QUOTE in their email.
    Fires Make.com Scenario 3 webhook → deposit invoice + agent notification.
    """
    token = request.args.get("token")
    if not token:
...
def client_changes():
    """
    Client clicks REQUEST CHANGES in their email.
    Step 1: Shows changes form.
    Step 2: On submit, fires Make.com Scenario 3 with change details.
    """
    token = request.args.get("token")
    if not token:
        return page_html("error", "Invalid Link", "This link is not valid.", "")
    ...
    # Form submitted — process changes
    ...
    if changes_text:
        ...
        return page_html(
            ...
            "Your agent will review your requests and send a revised quote shortly.",
        )
...
def page_html(status, title, message, sub, show_portal=False):
    ...
    portal_btn = (
        f'<br><br><a href="{PORTAL_URL}/quotes" '
        f'style="display:inline-block;background:{NAVY};color:white;'
        f'padding:11px 26px;border-radius:8px;text-decoration:none;'
        f'font-size:13px;font-weight:600;">Go to Portal →</a>'
    ) if show_portal else ""
...
<body>
  <div class="card">
    <div class="brand">SAFARIDESK</div>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <title>SafariDesk — Booking Confirmed!</title>
  <style>
    *{{margin:0;padding:0;box-sizing:border-box}}
    body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
      background:{NAVY};min-height:100vh;display:flex;align-items:center;
    ...
    .step-num{{background:{GOLD};color:white;width:22px;height:22px;
...
<body>
  <div class="card">
    ...
    <p class="sub">Your agent has been notified and will send deposit payment instructions shortly.</p>
    <div class="steps">
      <div style="font-size:10px;font-weight:700;letter-spacing:1px;
        color:#999;margin-bottom:12px;">WHAT HAPPENS NEXT</div>
      <div class="step">
      ...
      <div class="step">
        ...
        <div>Pay deposit via secure payment link to <b>confirm your booking</b></div>
      </div>
      <div class="step">
      ...
      <div class="step">
        ...
        <div><b>Balance payment</b> due before departure as per your agreement</div>
      </div>
    </div>
    <div class="agent-box">
      <div class="agent-label">YOUR TRAVEL SPECIALIST</div>
      <div class="agent-name">SafariDesk Travel</div>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <title>SafariDesk — Request Changes</title>
  <style>
    *{{margin:0;padding:0;box-sizing:border-box}}
    body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
      background:{NAVY};min-height:100vh;display:flex;align-items:center;
  ...
  <div class="card">
    ...
    <form method="GET" action="{API_BASE_URL}/client-changes">
      ...
      <select name="dates">
        ...
        <option value="Flexible by 1-2 weeks">Flexible by 1-2 weeks</option>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <title>SafariDesk — Reject Quote</title>
  <style>
    *{{margin:0;padding:0;box-sizing:border-box}}
    body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
      background:{NAVY};min-height:100vh;display:flex;align-items:center;
