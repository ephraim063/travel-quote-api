...
def generate_token(quote_id, action):
    ...
    sig = hmac.new(APPROVAL_SECRET.encode(), payload.encode(), hashlib.sha256).hexdigest()[:16]
...
def verify_token(token, expected_action):
    try:
        ...
        expected_sig = hmac.new(APPROVAL_SECRET.encode(), expected_payload.encode(), hashlib.sha256).hexdigest()[:16]
...
def agent_approval_email_html(agent_name, agency_name, client_name, quote_number,
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="margin:8px 0 4px;">
    <tr><td align="center">
      <a href="{cost_breakdown_url}" style="display:inline-block;background-color:#1A1A1A;color:#C4922A;padding:14px 32px;border-radius:6px;text-decoration:none;font-weight:bold;font-size:13px;letter-spacing:1px;border:1px solid #C4922A;">
        📊 VIEW COST BREAKDOWN
      </a>
    </td></tr>
  </table>
...
def client_quote_email_html(client_name, agency_name, agent_name, agent_email, agent_phone,
                             quote_number, start_date, end_date, accept_url, changes_url,
                             brand_primary='#2E4A7A', brand_secondary='#C4922A'):
    return f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;font-family:Arial,sans-serif;background:#f4f4f4;">
...
def fetch_photos_for_itinerary(itinerary):
    ...
    threads = [threading.Thread(target=fetch_one, args=(dn, q)) for dn, q in queries.items()]
    for t in threads: t.start()
    for t in threads: t.join(timeout=20)
    logger.info(f"Photos fetched: {len(photo_cache)}/{len(queries)}")
    return photo_cache
    ...
    try:
        ...
        if event_type == 'user.created':
            ...
            new_agent = {
                ...
                'agency_name': f"{agent_name} Safaris", 'email': email,
                'brand_color_primary': '#2E4A7A', 'brand_color_secondary': '#C4922A',
                'deposit_percentage': 30, 'balance_due_days': 60,
                'markup_type': 'overall', 'markup_overall_pct': 20,
                'subscription_status': 'trial', 'subscription_plan': 'free',
            }
            req = urllib.request.Request(f"{SUPABASE_URL}/rest/v1/agents",
                data=json.dumps(new_agent).encode('utf-8'), method='POST',
...
def generate_pdf():
    try:
        ...
        def cents(val): return round((val or 0) / 100, 2)
        ...
        claude_prompt = f"""You are a safari itinerary pricing specialist. Build the optimal itinerary using ONLY the exact pricing data provided below.
Return ONLY a valid JSON object. No explanation. No markdown. No code fences. Start with {{ and end with }}.
Structure:
{{"itinerary":[{{"day_number":1,"date":"YYYY-MM-DD","destination":"string","title":"string","accommodation_name":"string","room_type":"string","meal_plan":"string","nights":1,"transport_description":"string or null","image_search_query":"string"}}],"line_items":[{{"line_type":"accommodation|transport|park_fee","description":"string","details":"string","quantity":2,"unit_price":1950.00,"total_price":3900.00}}],"total_price_usd":0,"deposit_amount_usd":0,"balance_amount_usd":0,"within_budget":true,"budget_notes":"string or null"}}
...
TRIP REQUIREMENTS: {json.dumps({"destinations":destinations,"duration_days":duration_days,"start_date":start_date,"end_date":end_date,"pax_adults":pax_adults,"pax_children":pax_children,"accommodation_tier":accommodation_tier,"budget_usd":budget_usd,"special_requests":special_requests,"visitor_category":"non_resident"})}
...
PARK FEES: {json.dumps(fees_for_claude)}"""
        ...
        itinerary_data = call_claude(claude_prompt, max_tokens=4000)
        itinerary      = itinerary_data.get('itinerary', [])
        line_items     = itinerary_data.get('line_items', [])

        # ── Apply markup — saves cost data for agent breakdown ────────────────
        ...
        # ── Generate internal cost breakdown PDF ──────────────────────────────
        from pdf_generator import generate_cost_breakdown_pdf
        cb_filename    = f"SafariFlow_CostBreakdown_{quote_number}.pdf"
        cb_output_path = os.path.join(OUTPUT_DIR, cb_filename)
        generate_cost_breakdown_pdf(pdf_data, cb_output_path)
...
<div style="max-width:500px;margin:60px auto;background:white;padding:40px;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,0.1);text-align:center;">
    ...
    <form method="POST" action="/{action}-confirm">
        ...
        <button type="submit" style="background:{button_color};color:white;border:none;padding:14px 32px;border-radius:6px;font-size:15px;font-weight:bold;cursor:pointer;margin-right:12px;">{button_label}</button>
        <a href="javascript:history.back()" style="display:inline-block;background:#ccc;color:white;padding:14px 24px;border-radius:6px;font-size:15px;font-weight:bold;text-decoration:none;">Cancel</a>
    </form>
</div></body></html>'''
...
def approve():
    ...
    return confirmation_page(token, 'approve', 'Approve This Quote',
...
def approve_confirm():
    ...
    if quotes:
        ...
        if client_email_addr:
            pdf_url = quote.get('pdf_url', ''); pdf_base64 = ''
            if pdf_url:
                try:
...
def reject():
    ...
    if not quote_id: return invalid_page()
    return confirmation_page(token, 'reject', 'Request Changes',
...
def client_accept():
    ...
    if not quote_id: return invalid_page()
    return confirmation_page(token, 'client-accept', 'Accept This Quote',
...
<script>
  function toggleChange(btn,type){{btn.classList.toggle('selected');btn.querySelector('input[type="hidden"]').value=btn.classList.contains('selected')?'yes':'no';var f=document.getElementById('field_'+type);if(f)f.classList.toggle('visible',btn.classList.contains('selected'));}}
  var adults=2,children=0;
  function updatePax(type,delta){{if(type==='adults'){{adults=Math.max(1,adults+delta);document.getElementById('adults_display').textContent=adults;document.getElementById('revised_adults_hidden').value=adults;}}else{{children=Math.max(0,children+delta);document.getElementById('children_display').textContent=children;document.getElementById('revised_children_hidden').value=children;}}}}
  function toggleExtra(btn){{btn.classList.toggle('selected');btn.querySelector('.extra-input').value=btn.classList.contains('selected')?'yes':'no';}}
</script></body></html>'''
...
def client_changes_confirm():
    ...
    selected_extras = [k.replace('extra_','') for k, v in request.form.items() if k.startswith('extra_') and v == 'yes']
    change_request  = {
    ...
    if quotes:
        ...
        agent_email_addr = agent.get('email', '')
        if agent_email_addr:
            bp = agent.get('brand_color_primary','#2E4A7A'); bs = agent.get('brand_color_secondary','#C4922A')
...
def upload_inventory(inventory_type):
    try:
        ...
        file.save(temp_path)
        wb = openpyxl.load_workbook(temp_path, data_only=True); ws = wb.active
        ...
        for row_num, row in enumerate(ws.iter_rows(min_row=5, values_only=True), start=5):
            ...
            try:
                if inventory_type == 'accommodations':
                    ...
                    if not record['name'] or not record['destination']: rows_skipped += 1; continue
                    ...
                    if not record['from_location'] or not record['to_location']: rows_skipped += 1; continue
                ...
                errors.append(f"Row {row_num}: {str(row_err)}"); rows_skipped += 1
